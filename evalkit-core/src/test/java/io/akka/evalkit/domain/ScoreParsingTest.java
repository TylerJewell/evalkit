 ...
 */
@DisplayName("Verdict.parseScore · reading a judge's reply")
class ScoreParsingTest {
    ...
    void compliantReplies(String reply) {
        assertThat(Verdict.parseScore(reply)).contains(8);
    }
    ...
    void tenIsNotOne() {
        ...
        assertThat(Verdict.parseScore("10/10")).contains(10);
    }
    ...
    void unreadableReplies(String reply) {
        // A default would be a fabricated judgement, and a fabricated 1 and a fabricated
        // 10 are both worse than an admission that the judge did not answer.
        assertThat(Verdict.parseScore(reply)).isEmpty();
    }
    ...
    void aChattyReplyIsMisread() {
        ...
        assertThat(Verdict.parseScore("1 of 5 criteria were met, so I score this 8"))
            .contains(1);
        assertThat(Verdict.parseScore("Section 3 of the policy applies. Score: 9"))
            .contains(3);
    ...
    void multiDigitNumbersAreNotSplit() {
        ...
        assertThat(Verdict.parseScore("the policy changed in 2024")).isEmpty();
        assertThat(Verdict.parseScore("a refund of 240 euros")).isEmpty();
        assertThat(Verdict.parseScore("30 days")).isEmpty();
    }
    ...
    void anyInputIsSafe() {
        ...
        for (int i = 0; i < 20_000; i++) {
            ...
            var parsed = Verdict.parseScore(text.toString());

            // Every score reaching Verdict must be one Band.of can hold. A value outside
