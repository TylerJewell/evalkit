 ...
 * asserts the score that follows. The judgements here are built directly, which tests the
...
class TurnRelevancyConformanceTest {
    ...
    private static List<Judgement> judgements(int irrelevant, int relevant) {
        var out = new ArrayList<Judgement>();
        for (int i = 0; i < irrelevant; i++) {
            out.add(Judgement.denied("exchange " + out.size(), "scripted irrelevancy"));
        }
        for (int i = 0; i < relevant; i++) {
            out.add(Judgement.affirmed("exchange " + out.size()));
        }
        return out;
    }
    ...
    void scoreIsTheShareOfRelevantExchanges(int exchanges, int irrelevant, double expected) {
        var scripted = judgements(irrelevant, exchanges - irrelevant);
    ...
    @Test
    @DisplayName("no readable judgement scores 1, which is the documented fallback")
    ...
    void oneIrrelevantExchange() {
        assertThat(metric.aggregate(judgements(1, 0))).isEqualTo(0.0);
    }

    @Test
    @DisplayName("half relevant scores 0.5")
    void halfRelevant() {
        assertThat(metric.aggregate(judgements(1, 1))).isEqualTo(0.5);
    }

    @Test
    @DisplayName("the order judgements arrive in does not change the score")
    void orderDoesNotMatter() {
        var denialFirst = List.of(Judgement.denied("a", "no"), Judgement.affirmed("b"));
        var denialLast = List.of(Judgement.affirmed("b"), Judgement.denied("a", "no"));
